  ...
  mutations: {
    ...
    LOAD_HEROE_DETAILS: (state, payload) => {
      ...
      localStorage.setItem(
        `${payload[0].id}detail-vue`,
        JSON.stringify(state.heroeDetails)
      );
    },
    ...
    SEARCH_HEROE: (state, payload) => {
      state.heroes = payload;
    },
    UPDATE_HEROE: (state, payload) => {
      state.heroes = payload;
    },
  },
  actions: {
    ...
    GET_DETAILS: function (state, payload) {
      let detailExist = JSON.parse(
        localStorage.getItem(`${payload}detail-vue`)
      );
    ...
    PUT_HEROE: function (state, payload) {
      console.log(payload);
      const found = this.state.heroes.findIndex(
    ...
    SEARCH_HEROE: function (state, payload) {
      ...
      const search = allHeroes.filter((element) =>
